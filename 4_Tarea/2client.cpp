...
class Client {
private:
    std::string nickname, dest_nickname;
...
public:
    Client(std::string _nickname, std::string _dest_nickname) {
    ...
    void writeFunction(int socketFD) {
        for (;;) {
            ...
            packageManager.sendPacket(socketFD, dest_nickname, msg);
        }
    }
...
int main(void) {
    std::string client_nickname, dest_nickname;
    std::cout << "Enter nickname: ";
    std::getline(std::cin, client_nickname);

    std::cout << "Enter destinatary nickname: ";
    std::getline(std::cin, dest_nickname);

	Client client(client_nickname,dest_nickname);
    client.init();
